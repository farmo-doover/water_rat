...
class ui_manager:
    ...
    def get_cmd_defaults(self):
        ...
        for c in all_children:
            if hasattr(c, 'get_default_value'):
                cmds[c.get_name()] = c.get_default_value()

        return cmds

    def set_default_commands(self):
        cmds = self.get_available_commands()
        cmd_defaults = self.get_cmd_defaults()
        for cmd, value in cmd_defaults.items():
            if cmd not in cmds:
                self.coerce_command(cmd, value)
