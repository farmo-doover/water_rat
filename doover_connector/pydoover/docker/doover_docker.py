...
class app_base:
    ...
    def __init__(self, manager=None, name=None):
        ...
        self.platform_iface: Optional[platform_iface] = None
    ...
    def get_ui_manager(self):
        if self.ui_manager is None:
            self.ui_manager: UIManager = UIManager(agent_id=None, client=self.get_agent_iface())
    ...
    def get_platform_iface(self):
        if self.platform_iface is None:
            self.platform_iface: platform_iface = self.get_manager().get_platform_iface()
    ...
    async def recv_updates(self, channel_name, last_aggregate):
        ...
        if self.update_callback is not None:
            if asyncio.iscoroutinefunction(self.update_callback):
                await self.update_callback(self.last_deployment_config)
            else:
                self.update_callback(self.last_deployment_config)
...
def run_app(app, dda_iface=None, plt_iface=None, mb_iface=None, cam_iface=None, tunnel_iface: TunnelInterface = None, debug=False):
    ...
    if tunnel_iface is None:
        tunnel_iface = TunnelInterface(tunnel_uri)
