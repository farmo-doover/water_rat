...
class device_agent_iface:
    ...
    async def recv_update_callback(self, channel_name, response):
        ...
        aggregate, valid = self._get_channel_aggregate(channel_name)
        if not valid:
        ...
        for callback in self.subscriptions[channel_name]["callbacks"]:
            if asyncio.iscoroutinefunction(callback):
                await callback(channel_name, aggregate)
            else:
            ...
            while True:
                try:
                    ...
                    if not response.response_header.success:
                        ...
                        if asyncio.iscoroutinefunction(callback):
                            await callback(channel_name, response)
                        else:
    ...
    def _get_channel_aggregate(self, channel_name) -> tuple[str, bool]:
        with grpc.insecure_channel(self.dda_uri) as channel:
            ...
            self.update_dda_status(response.response_header)
            return response.channel.aggregate, response.response_header.success

    def get_channel_aggregate(self, channel_name):
        aggregate, valid = self._get_channel_aggregate(channel_name)
        if not valid:
            logging.error(f"Failed to get channel aggregate: {channel_name}, "
                          f"no valid response was returned from the server.")
            return None
    ...
    def get_temp_token(self) -> tuple[str, datetime.datetime, str]:
        with grpc.insecure_channel(self.dda_uri) as channel:
            stub = device_agent_pb2_grpc.deviceAgentStub(channel)
            response = stub.GetTempAPIToken(device_agent_pb2.TempAPITokenResponse(), timeout=self.dda_timeout)
