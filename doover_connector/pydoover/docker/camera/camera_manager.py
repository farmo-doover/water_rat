...
from ...ui import Camera, SlimCommand
...
class camera_manager:
    ...
    def fetch_ui_elements(self):
        ...
        return [Camera(c.get_name(), c.get_display_name(), c.get_uri()) for c in self.get_cameras()]
    ...
    def setup(self):
        ...
        self.ui_manager._add_interaction(SlimCommand("camera_snapshots", callback=self.on_snapshot_command))
    ...
    def on_snapshot_command(self, new_value: str):
    ...
    async def is_snapshot_running(self):
        return self.app_manager.get_camera_iface().is_snapshot_running()
