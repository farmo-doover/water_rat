...
class camera_iface:
    ...
    def is_snapshot_running(self):
        try:
            with grpc.insecure_channel(self.camera_iface_uri) as channel:
                stub = camera_iface_pb2_grpc.cameraIfaceStub(channel)
                response = stub.IsSnapshotRunning(camera_iface_pb2.IsSnapshotRunningRequest())
                if response is None:
                    logging.warning("Error processing is_snapshot_running response: " + str(response))
                elif not response.response_header.success:
                    logging.warning("Error processing is_snapshot_running response: " + str(response.response_header.message))
                else:
                    return response.is_running

        except Exception as e:
            logging.error("Error getting is snapshot running: " + str(e))
